...
class OnlyOneOfConstraint extends Constraint {
  ...
  toString () {
    ...
    return `<Node can have one of ${this.fields} fields${conds}>`
  }
...
class FieldTypeConstraint extends Constraint {
  ...
  valueCheck (value, path, problems) {
    switch (this.type) {
      case 'object':
        if (typeof value !== 'object' && !Array.isArray(value)) this.report(path, value, 'an Object', problems)
        break
      case 'array':
        if (!Array.isArray(value)) this.report(path, value, 'an Array', problems)
        break
      case 'string':
        if (typeof value !== 'string') this.report(path, value, 'a String', problems)
        break
      case 'integer':
        if (!Number.isInteger(value)) this.report(path, value, 'an Integer', problems)
        break
      case 'float':
        if (typeof value !== 'number' || Number.isInteger(value)) this.report(path, value, 'a Float', problems)
        break
      case 'boolean':
        if (typeof value !== 'boolean') this.report(path, value, 'a Boolean', problems)
        break
      case 'json_path':
        break
      case 'reference_path':
      ...
      case 'timestamp':
        ...
        if (!ts.isValid) this.report(path, value, 'an ISO8601 timestamp', problems) // Spec actually says RFC3339, but close enough
        break
      case 'URI':
        if ((typeof value !== 'string') || (!value.match(/^[a-z]+:/))) this.report(path, value, 'a URI', problems)
        break
    }
  } // valueCheck
...
class FieldValueConstraint extends Constraint {
  ...
  toString () {
    const conds = this.conditions.length ? ` ${this.conditions.length} conditions` : ''
    return `<Field ${this.names} has constraints ${this.params}${conds}>`
  } // toString
